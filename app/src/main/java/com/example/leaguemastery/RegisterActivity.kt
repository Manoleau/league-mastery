...
class RegisterActivity : AppCompatActivity() {
    ...
    private fun initializeUI() {
        ...
        register.setOnClickListener(View.OnClickListener { createNewAccount() })
            ...
            mAuth.createUserWithEmailAndPassword(email, pwd)
                .addOnCompleteListener(OnCompleteListener<AuthResult?> { task ->
                    if (task.isSuccessful) //If account creation successful print message and send user to Login Activity
                    {
                        ...
                        val updateProfile = UserProfileChangeRequest.Builder().setDisplayName(userName.text.toString()).build()
                    ...
                    {
                        Toast.makeText(this@RegisterActivity, "Error: ${task.exception?.message}", Toast.LENGTH_SHORT)
                            .show()
                        loadingBar.dismiss()
                    }
                })
